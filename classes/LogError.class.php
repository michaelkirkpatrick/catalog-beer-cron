...
class LogError {
	...
	public function write(){
		...
		if(!$uuid->error){
			...
			// Add to Database
			$query = "INSERT INTO error_log (id, errorNumber, errorMessage, badData, URI, ipAddress, timestamp, filename, resolved) VALUES('$dbErrorID', '$dbErrorNumber', '$dbErrorMessage', '$dbBadData', '', '', '$dbTimestamp', '$dbFilename', '0')";
			$db->query($query);
			
			// Troubleshooting
			echo "Error Found: ($dbErrorNumber): $dbErrorMessage [$dbBadData]. ($dbFilename)\n";
		}
	}
	...
	public function resolved($errorID){
		if(!empty($errorID)){
			...
			$this->badData = '';
			$this->filename = 'CRON/LogError.class.php';
			$this->write();
		}
	}
}
